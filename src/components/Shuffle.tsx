    ...
    () => {
      ...
      const build = () => {
        ...
        chars.forEach(ch => {
          ...
          Object.assign(firstOrig.style, { display: 'inline-block', width: w + 'px', textAlign: 'center' } as CSSStyleDeclaration);

          ch.setAttribute('data-orig', '1');
          Object.assign(ch.style, { display: 'inline-block', width: w + 'px', textAlign: 'center' } as CSSStyleDeclaration);

          inner.appendChild(firstOrig);
          for (let k = 0; k < rolls; k++) {
            const c = ch.cloneNode(true) as HTMLElement;
            if (scrambleCharset) c.textContent = rand(scrambleCharset);
            Object.assign(c.style, { display: 'inline-block', width: w + 'px', textAlign: 'center' } as CSSStyleDeclaration);
            inner.appendChild(c);
          }
      ...
      const cleanupToStill = () => {
        wrappersRef.current.forEach(w => {
          ...
          (strip.style as any).transform = 'none';
          (strip.style as any).willChange = 'auto';
        });
      };
